...
class Answer {
  ...
  async getAnswerData (AnsId) {
    const statusvalue = 1
    const userExist = await db.AnswerManager
      .findOne({
        where: {
          AnsId: AnsId
        },
    ...
    const userData = (!_.isEmpty(userExist)) ? { AnsId: userExist.AnsId, AnsDescription: userExist.AnsDescription,QuestionId:userExist.QuestionId,IsValid:userExist.IsValid } : {}
    return userData
  }
...
const Answerdata = async (req, res) => {
  try {
    ...
    const answerdata= await AnswerCls.getAnswerData(AnsId)
