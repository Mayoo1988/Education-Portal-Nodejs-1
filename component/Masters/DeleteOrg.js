...
class User {
    ...
    return new Promise((resolve, reject) => {
        db.sequelize.query(`CALL Sp_delete_Org(:_OrgId)`,
          {
            replacements: {
                _OrgId: OrgId
            }//,
...
const orgdelete = async (req, res) => {
  ...
  //  console.log(passedUserId)
    ...
    if (legit.userId == passedUserId)
   {
     ...
     const userdelete = await UserCls.deleteOrgInDB(orgId)
     res.status(200).send(apiResponse.successFormat(`success`, `delete successful`, userdelete, []))  
    } 
    else
     {
